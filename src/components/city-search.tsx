...
const CitySeatch = () => {
  ...
  const handleSelectChange = (cityData: string) => {
    ...
    const [lat, long, name, country] = cityData.split("|");
    console.log(lat, long, name, country);
    // 做某些事情的時候切換到Count頁面
    navigate(`/city/${name}??lat=${lat}&lon=${long}`); // 例如： 搜尋 new taipei => New%20Taipei??lat=25.012&lon=121.45 => %20 === 空格
  };
  return (
    <>
      <Button
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandList>
          ...
          <CommandSeparator />
          <CommandGroup heading="Recent Seatches">
            <CommandItem>Calendar</CommandItem>
          </CommandGroup>
          <CommandSeparator />
